...
var NewsFeedView = /** @class */function (_super) {
  ...
  NewsFeedView.prototype.render = function () {
    ...
    for (var i = (store.currentPage - 1) * this.limit; i < store.currentPage * this.limit; i++) {
      var _a = this.feeds[i],
        id = _a.id,
        title = _a.title,
    ...
    this.setTemplateData('prev_page', "".concat(store.currentPage > 1 ? store.currentPage - 1 : 1));
    this.setTemplateData('next_page', "".concat(isMaxPage ? store.currentPage : store.currentPage + 1));
    this.updateView();
  };
